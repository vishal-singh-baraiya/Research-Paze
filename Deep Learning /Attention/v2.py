...
BLOCK_SIZE = 16
BTREE_ORDER = 5  # Increased to avoid empty key lists
TRAINING_EPOCHS = 200
LEARNING_RATE = 5e-4
DROPOUT = 0.1
PATIENCE = 10
...
class BTreeAttentionIndex:
    ...
    def _insert_non_full(self, node: BTreeNode, key: int, value: torch.Tensor):
        if node.is_leaf:
            ...
            if i >= len(node.children):
                logger.error(f"Index {i} out of bounds for node.children with length {len(node.children)}")
                return
            if node.children[i].is_full():
                ...
                if i < len(node.keys) and key > node.keys[i]:
                    i += 1
                if i >= len(node.children):
                    logger.error(f"Index {i} still out of bounds after split for node.children with length {len(node.children)}")
                    return
    ...
    def _split_child(self, parent: BTreeNode, index: int):
        ...
        if new_child.keys:
            ...
            logger.warning(f"New child has no keys after split, using last key from full child: {full_child.keys[-1] if full_child.keys else 'None'}")
            parent.keys.insert(index, full_child.keys[-1] if full_child.keys else 0)  # Fallback to 0 if no keys
                ...
                for i, key in enumerate(node.keys):
                    if start <= key:
                        if i < len(node.children):
                            ...
                            logger.error(f"Invalid child index {i} for node with {len(node.children)} children")
                    if key > end:
                        break
...
class EnhancedTransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, n_heads, block_size, use_btree_optimization=True, device='cuda'):
        super().__init__()
        self.use_btree = use_btree_optimization
        if use_btree_optimization:
...
class OptimizedSimpleTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, d_model, n_heads, block_size, use_btree=True, device='cuda'):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            EnhancedTransformerBlock(d_model, n_heads, block_size, use_btree, device=device)
    ...
    def get_model_stats(self):
        ...
        for i, layer in enumerate(self.layers):
            if self.use_btree and hasattr(layer.attention, 'get_cache_stats'):
...
def train_model_enhanced(model_name, model, train_loader, val_loader, device='cuda'):
...
if __name__ == '__main__':
    ...
    if device == 'cpu':
        print("Warning: GPU not available, falling back to CPU. Performance may be suboptimal.")
    else:
    ...
    try:
        ...
        raise Exception(f"Error reading {data_file}: {str(e)}")
    ...
    btree_model = OptimizedSimpleTransformer(
        vocab_size=vocab_size, num_layers=2, d_model=D_MODEL, n_heads=N_HEADS,
        block_size=BLOCK_SIZE, use_btree=True, device=device
    ).to(device)
    train_model_enhanced("B+ Tree Optimized PHA Model", btree_model, train_loader, val_loader, device)

    # Train Standard Model
    standard_model = OptimizedSimpleTransformer(
        vocab_size=vocab_size, num_layers=2, d_model=D_MODEL, n_heads=N_HEADS,
        block_size=BLOCK_SIZE, use_btree=False, device=device
    ).to(device)
    train_model_enhanced("Standard Attention Model", standard_model, train_loader, val_loader, device)

    print("=== Final Model Statistics ===")
    btree_stats = btree_model.get_model_stats()
    for layer_name, stats in btree_stats.items():
        print(f"{layer_name}: Hit Rate = {stats['hit_rate']:.3f}, "
              f"B+ Tree Height = {stats['btree_height']}")
    ...
    btree_generated = btree_model.generate(prompt, max_new_tokens=50, temperature=0.5, top_k=50)
    print("Generated:", decode_indices(btree_generated[0], reverse_vocab))
    
    print("\nGenerating with Standard Attention Model:")
    standard_generated = standard_model.generate(prompt, max_new_tokens=50, temperature=0.5, top_k=50)
    print("Generated:", decode_indices(standard_generated[0], reverse_vocab))
