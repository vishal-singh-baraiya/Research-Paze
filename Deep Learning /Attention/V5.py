...
BLOCK_SIZE = 16  # Block size for hierarchical attention
...
class TextDataset(Dataset):
    """Custom Dataset for loading text sequences with Hugging Face tokenizer"""
...
class OptimizedSimpleTransformer(nn.Module):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.0, top_k=None, top_p=None, repetition_penalty=1.0):
        """
        Generate text sequences with top-k, top-p sampling, and repetition penalty.
        ...
        Args:
            ...
            top_p: If specified, only consider tokens in the smallest set with cumulative probability > p
        ...
        self.eval()
        generated = idx.clone()
        generated_tokens = set(generated[0].tolist())  # Track generated tokens for penalty
        for step in range(max_new_tokens):
            ...
            if top_k is not None or top_p is not None:
                v, idx_top = torch.topk(logits, min(top_k or self.vocab_size, self.vocab_size))
                logits = torch.full_like(logits, -float('Inf')).scatter_(-1, idx_top, v)
                if top_p is not None:
                    ...
                    logits = logits.scatter(-1, sorted_indices, sorted_logits.masked_fill(sorted_indices_to_remove, -float('Inf')))
            probs = F.softmax(logits, dim=-1)
            next_token = torch.multinomial(probs, num_samples=1)
            generated = torch.cat((generated, next_token), dim=1)
            generated_tokens.add(next_token[0].item())
            # Log token diversity every 10 steps
            if (step + 1) % 10 == 0:
                logger.info(f"Generation step {step + 1}: Unique tokens = {len(generated_tokens)}")
        return generated

def prepare_data(text, tokenizer, seq_len=64):
    """Prepare data using Hugging Face tokenizer"""
    tokens = tokenizer(text, return_tensors='pt', truncation=True, max_length=seq_len*1000)['input_ids'][0]
    return tokens
...
def decode_indices(indices, tokenizer):
    """Convert indices back to text using Hugging Face tokenizer"""
...
if __name__ == '__main__':
    ...
    try:
        dataset = load_dataset("deven367/babylm-100M-children-stories", split='train')
    except Exception as e:
        raise ValueError(f"Error loading dataset: {str(e)}")

    # Extract text data (assuming 'text' column exists)
    story_text = " ".join(dataset['text'])  # Concatenate all text entries
    if not story_text:
        raise ValueError("Error: Dataset is empty or no text data found.")

    # Tokenize data
    indices = prepare_data(story_text, tokenizer, seq_len=)
    vocab_size = tokenizer.vocab_size
    dataset = TextDataset(indices, seq_len=16)
    ...
    prompt = sample_idx[:50].unsqueeze(0).to(device)
    print("Prompt:", decode_indices(prompt[0], tokenizer))
    
    print("\nGenerating with B+ Tree Optimized Model (Top-k + Top-p + Repetition Penalty):")
    ...
    print("Generated:", decode_indices(generated[0], tokenizer))
