...
BLOCK_SIZE = 16  # Reduced block size for finer context
...
BATCH_SIZE = 32
GRAD_ACCUM_STEPS = 4
MAX_CACHE_SIZE = 20  # Reduced cache size
...
class OptimizedSimpleTransformer(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, d_model, n_heads, block_size, device='cuda'):
        ...
        self.pos_encoding = self.get_sinusoidal_pos_encoding(512, d_model).to(device)
    ...
    def get_sinusoidal_pos_encoding(self, seq_len, d_model):
    ...
    def generate(self, idx, max_new_tokens, temperature=1.2, top_k=200, top_p=None):
        """
        Generate text sequences with top-k or top-p sampling.
        
        Args:
            idx: Input indices (batch_size, seq_len)
            max_new_tokens: Maximum number of tokens to generate
            temperature: Sampling temperature
            top_k: Number of top tokens to sample from (if specified)
            top_p: Cumulative probability for nucleus sampling (if specified)
        ...
        for i in range(max_new_tokens):
            ...
            logger.info(f"Step {i+1} Top 5 Tokens: {[reverse_vocab[idx.item()] for idx in top_indices[0]]}")
            next_token = torch.multinomial(probs, num_samples=1)
            generated = torch.cat((generated, next_token), dim=1)
        return generated

    def beam_search(self, idx, max_new_tokens, beam_width=5):
        """
        Generate text sequences using beam search.
        ...
        for _ in range(max_new_tokens):
            ...
            for seq, log_prob in beams:
                idx_cond = seq[:, -self.pos_encoding.size(1):]
                with torch.no_grad():
                    logits = self(idx_cond)[:, -1, :]
                probs = F.softmax(logits, dim=-1)
                top_probs, top_indices = torch.topk(probs, beam_width)
                for i in range(beam_width):
                    new_seq = torch.cat((seq, top_indices[:, [i]]), dim=1)
...
if __name__ == '__main__':
    ...
    tokens = re.findall(r'\b\w+\b', story_text.lower())
    print("Top 5 tokens:", Counter(tokens).most_common(5))
    ...
    prompt = sample_idx[:20].unsqueeze(0).to(device)  # Longer prompt
    print("Prompt:", decode_indices(prompt[0], reverse_vocab))
    
    print("\nGenerating with Sampling (Top-k + Top-p):")
    generated = model.generate(prompt, max_new_tokens=50, temperature=1.2, top_k=200, top_p=0.9)
    print("Generated:", decode_indices(generated[0], reverse_vocab))

    print("\nGenerating with Beam Search:")
    generated_beam = model.beam_search(prompt, max_new_tokens=50, beam_width=5)
    print("Generated:", decode_indices(generated_beam[0], reverse_vocab))
